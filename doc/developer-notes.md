...
- **`js/personalDataPage.js`**:Lv40-50 全曲データを取得する。公式サイトへの負荷配慮として、
  ...
  - **差分履歴機能**:「データ更新」ボタン押下時、更新前のデータと更新後のデータを
    `computePersonalDataDiff()`で比較し、スコア/メダル/ランクが変化した曲を`localStorage`
    (`personal_diff_history`キー)に更新日ごとの履歴として蓄積する。曲の合計件数が
    `DIFF_HISTORY_MAX_RECORDS`(既定500)を超えたら古い更新日から間引く(直近の更新分は
    単体で上限を超えていても残す)。個人統計ページの「前回からの変化」セクションで、
    件数サマリ・詳細テーブル・過去の更新日を選んで見返せるプルダウンを表示する。
